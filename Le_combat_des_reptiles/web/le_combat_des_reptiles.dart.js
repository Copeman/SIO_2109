...
$$.JSNumber = {"": "Interceptor;",
  ...
  get$ceil: function(receiver) {
    return new $.BoundClosure$i0(this, "ceil$0", receiver);
  },
  toString$0: function(receiver) {
...
$$.game = {"": "Object;canvas>,context<,enemy<,player<,keys,lastKey,over@",
  ...
  stop$1: function(_, cycle) {
    ...
    t1 = this.canvas;
    t2 = $.getInterceptor$x(t1);
    height2 = $.$div$n(t2.get$height(t1), 15);
    this.context.font = $.JSString_methods.$add($.S(height2), "px sans-serif");
    this.context.textAlign = "center";
    winner = $.$eq(cycle.$index(cycle, "type"), "program") === true ? "gagn\u00e9" : "perdu";
    this.context.fillText($.JSString_methods.$add("Partie termin\u00e9e - Vous avez ", winner), $.$div$n(t2.get$width(t1), 2), $.$div$n(t2.get$height(t1), 2));
    t3 = this.context;
    t4 = $.$div$n(t2.get$width(t1), 2);
    t1 = $.$div$n(t2.get$height(t1), 2);
    t2 = $.$mul$n(cycle.$index(cycle, "height"), 3);
    if (typeof t2 !== "number")
      throw $.iae(t2);
    t3.fillText("Appuyez sur espace pour continuer.", t4, t1 + t2);
    cycle.$indexSet(cycle, "patern", "body3");
  },
  resetPlayer$0: function() {
    ...
    t1.$indexSet(t1, "x", t5.$sub(t4, t5.$div(t4, $.$div$n(t1.$index(t1, "width"), 2)) + 8));
    t1.$indexSet(t1, "y", $.$div$n(t3.get$height(t2), 2) + $.$div$n(t1.$index(t1, "height"), 2));
    t1.$indexSet(t1, "patern", "body");
    t1.$indexSet(t1, "history", []);
    t1.$indexSet(t1, "current_direction", "left");
  },
  resetEnemy$0: function() {
    var t1, t2, t3;
    t1 = this.enemy;
    t2 = this.canvas;
    t3 = $.getInterceptor$x(t2);
    t1.$indexSet(t1, "x", $.$div$n(t3.get$width(t2), $.$div$n(t1.$index(t1, "width"), 2)) - 8);
    t1.$indexSet(t1, "y", $.$div$n(t3.get$height(t2), 2) + $.$div$n(t1.$index(t1, "height"), 2));
    t1.$indexSet(t1, "patern", "body2");
    t1.$indexSet(t1, "history", []);
    t1.$indexSet(t1, "current_direction", "right");
  },
  move$2: function(cycle, opponent) {
    var test1, test2, coords;
    switch (cycle.$index(cycle, "current_direction")) {
      case "up":
        cycle.$indexSet(cycle, "y", $.$sub$n(cycle.$index(cycle, "y"), cycle.$index(cycle, "height")));
        break;
      case "down":
        cycle.$indexSet(cycle, "y", $.$add$ns(cycle.$index(cycle, "y"), cycle.$index(cycle, "height")));
        break;
      case "right":
        cycle.$indexSet(cycle, "x", $.$add$ns(cycle.$index(cycle, "x"), cycle.$index(cycle, "width")));
        break;
      case "left":
        cycle.$indexSet(cycle, "x", $.$sub$n(cycle.$index(cycle, "x"), cycle.$index(cycle, "width")));
        break;
    }
    if (this.checkCollision$2(cycle, opponent))
      this.stop$1(this, cycle);
    test1 = cycle.$index(cycle, "x");
    test2 = cycle.$index(cycle, "y");
    coords = $.S(test1) + "," + $.S(test2);
    $.add$1$ax(cycle.$index(cycle, "history"), coords);
  },
  moveEnemy$0: function() {
    var advisor, t1, t2;
    advisor = this.enemyPingDirections$0();
    t1 = this.enemy;
    if (!$.$lt$n(advisor.$index(advisor, t1.$index(t1, "current_direction")), t1.$index(t1, "width"))) {
      $.JSNumber_methods.get$ceil($.C__Random.nextDouble$0() * 10);
      t2 = false;
    } else
      t2 = true;
    if (t2)
      t1.$indexSet(t1, "current_direction", advisor.$index(advisor, "best"));
    this.move$2(t1, this.player);
  },
  enemyPingDirections$0: function() {
    var pong, t1, i, t2, t3, t4, t5, largest, j;
    pong = $.makeLiteralMap(["up", 0, "down", 0, "left", 0, "right", 0]);
    t1 = this.enemy;
    i = $.$sub$n(t1.$index(t1, "y"), t1.$index(t1, "height"));
    if (typeof i !== "number")
      return this.enemyPingDirections$0$bailout(1, pong, t1, i);
    while (i >= 0) {
      pong.$indexSet(pong, "up", $.$sub$n($.$sub$n(t1.$index(t1, "y"), i), t1.$index(t1, "width")));
      if (this.isCollision$2(t1.$index(t1, "x"), i))
        break;
      t2 = t1.$index(t1, "height");
      if (typeof t2 !== "number")
        throw $.iae(t2);
      i -= t2;
    }
    i = $.$add$ns(t1.$index(t1, "y"), t1.$index(t1, "height"));
    if (typeof i !== "number")
      return this.enemyPingDirections$0$bailout(2, pong, t1, i);
    while (t2 = this.canvas, t3 = $.getInterceptor$x(t2), $.JSNumber_methods.$le(i, t3.get$height(t2))) {
      t4 = t1.$index(t1, "y");
      if (typeof t4 !== "number")
        throw $.iae(t4);
      t5 = t1.$index(t1, "width");
      if (typeof t5 !== "number")
        throw $.iae(t5);
      pong.$indexSet(pong, "down", i - t4 - t5);
      if (this.isCollision$2(t1.$index(t1, "x"), i))
        break;
      t2 = t1.$index(t1, "height");
      if (typeof t2 !== "number")
    ...
    i = $.$sub$n(t1.$index(t1, "x"), t1.$index(t1, "width"));
    if (typeof i !== "number")
      return this.enemyPingDirections$0$bailout(3, pong, t1, i, t2, t3);
    while (i >= 0) {
      pong.$indexSet(pong, "left", $.$sub$n($.$sub$n(t1.$index(t1, "x"), i), t1.$index(t1, "width")));
      if (this.isCollision$2(i, t1.$index(t1, "y")))
        break;
      t4 = t1.$index(t1, "width");
      if (typeof t4 !== "number")
        throw $.iae(t4);
      i -= t4;
    }
    i = $.$add$ns(t1.$index(t1, "x"), t1.$index(t1, "width"));
    if (typeof i !== "number")
      return this.enemyPingDirections$0$bailout(4, pong, t1, i, t2, t3);
    while ($.JSNumber_methods.$le(i, t3.get$width(t2))) {
      t4 = t1.$index(t1, "x");
      if (typeof t4 !== "number")
        throw $.iae(t4);
      t5 = t1.$index(t1, "width");
      if (typeof t5 !== "number")
        throw $.iae(t5);
      pong.$indexSet(pong, "right", i - t4 - t5);
      if (this.isCollision$2(i, t1.$index(t1, "y")))
        break;
      t4 = t1.$index(t1, "width");
      if (typeof t4 !== "number")
    ...
    largest = $.makeLiteralMap(["key", null, "value", 0]);
    for (t1 = pong.get$keys(), t1 = t1.get$iterator(t1); t1.moveNext$0();) {
      j = t1.get$current();
      if ($.$gt$n(pong.$index(pong, j), largest.$index(largest, "value"))) {
        largest.$indexSet(largest, "key", j);
        largest.$indexSet(largest, "value", pong.$index(pong, j));
      }
    }
    pong.$indexSet(pong, "best", largest.$index(largest, "key"));
    return pong;
  },
  enemyPingDirections$0$bailout: function(state0, pong, t1, i, t2, t3) {
    switch (state0) {
      case 0:
        pong = $.makeLiteralMap(["up", 0, "down", 0, "left", 0, "right", 0]);
        t1 = this.enemy;
      ...
      case 1:
        state0 = 0;
        for (; t2 = $.getInterceptor$n(i), t2.$ge(i, 0); i = t2.$sub(i, t1.$index(t1, "height"))) {
          pong.$indexSet(pong, "up", $.$sub$n($.$sub$n(t1.$index(t1, "y"), i), t1.$index(t1, "width")));
          if (this.isCollision$2(t1.$index(t1, "x"), i))
            break;
        }
      ...
      case 2:
        state0 = 0;
        for (; t2 = this.canvas, t3 = $.getInterceptor$x(t2), t4 = $.getInterceptor$n(i), t4.$le(i, t3.get$height(t2)); i = t4.$add(i, t1.$index(t1, "height"))) {
          pong.$indexSet(pong, "down", $.$sub$n(t4.$sub(i, t1.$index(t1, "y")), t1.$index(t1, "width")));
          if (this.isCollision$2(t1.$index(t1, "x"), i))
            break;
        }
        i = $.$sub$n(t1.$index(t1, "x"), t1.$index(t1, "width"));
      case 3:
        state0 = 0;
        for (; t4 = $.getInterceptor$n(i), t4.$ge(i, 0); i = t4.$sub(i, t1.$index(t1, "width"))) {
          pong.$indexSet(pong, "left", $.$sub$n($.$sub$n(t1.$index(t1, "x"), i), t1.$index(t1, "width")));
          if (this.isCollision$2(i, t1.$index(t1, "y")))
            break;
        }
      ...
      case 4:
        var t4, largest, j;
        state0 = 0;
        for (; t4 = $.getInterceptor$n(i), t4.$le(i, t3.get$width(t2)); i = t4.$add(i, t1.$index(t1, "width"))) {
          pong.$indexSet(pong, "right", $.$sub$n(t4.$sub(i, t1.$index(t1, "x")), t1.$index(t1, "width")));
          if (this.isCollision$2(i, t1.$index(t1, "y")))
            break;
        }
        largest = $.makeLiteralMap(["key", null, "value", 0]);
  ...
  checkCollision$2: function(cycle, opponent) {
    var t1, t2, test1, test2;
    if (!$.$lt$n(cycle.$index(cycle, "x"), $.$div$n(cycle.$index(cycle, "width"), 2))) {
      t1 = this.canvas;
      t2 = $.getInterceptor$x(t1);
      if (!$.$gt$n(cycle.$index(cycle, "x"), $.$sub$n(t2.get$width(t1), $.$div$n(cycle.$index(cycle, "width"), 2))))
        if (!$.$lt$n(cycle.$index(cycle, "y"), $.$div$n(cycle.$index(cycle, "height"), 2)))
          if (!$.$gt$n(cycle.$index(cycle, "y"), $.$sub$n(t2.get$height(t1), $.$div$n(cycle.$index(cycle, "height"), 2)))) {
            t1 = cycle.$index(cycle, "history");
            test1 = cycle.$index(cycle, "x");
            test2 = cycle.$index(cycle, "y");
            if (!$.$ge$n($.indexOf$1$asx(t1, $.S(test1) + "," + $.S(test2)), 0)) {
              t1 = opponent.$index(opponent, "history");
              test1 = cycle.$index(cycle, "x");
              test2 = cycle.$index(cycle, "y");
              test2 = $.$ge$n($.indexOf$1$asx(t1, $.S(test1) + "," + $.S(test2)), 0);
              t1 = test2;
            } else
              t1 = true;
          } else
  ...
  isCollision$2: function(x, y) {
    var coords, t1, t2, t3, t4, t5;
    if (typeof x !== "number")
      return this.isCollision$2$bailout(1, x, y);
    if (typeof y !== "number")
      return this.isCollision$2$bailout(1, x, y);
    coords = $.S(x) + "," + $.S(y);
    t1 = this.enemy;
    t2 = t1.$index(t1, "width");
    if (typeof t2 !== "number")
      return this.isCollision$2$bailout(2, x, y, t2, coords, t1);
    if (!(x < t2 / 2)) {
      t2 = this.canvas;
      t3 = $.getInterceptor$x(t2);
      t4 = t3.get$width(t2);
      if (typeof t4 !== "number")
        return this.isCollision$2$bailout(3, x, y, t2, coords, t1, $.JSNumber_methods, t3, t4);
      t5 = t1.$index(t1, "width");
      if (typeof t5 !== "number")
        return this.isCollision$2$bailout(4, x, y, t2, coords, t1, $.JSNumber_methods, t3, t4, t5);
      if (!(x > t4 - t5 / 2)) {
        t4 = t1.$index(t1, "height");
        if (typeof t4 !== "number")
          return this.isCollision$2$bailout(5, 0, y, t2, coords, t1, t4, t3);
        if (!(y < t4 / 2)) {
          t2 = t3.get$height(t2);
          if (typeof t2 !== "number")
            return this.isCollision$2$bailout(6, 0, y, t2, coords, t1, 0, 0, $.JSNumber_methods);
          t3 = t1.$index(t1, "height");
          if (typeof t3 !== "number")
            return this.isCollision$2$bailout(7, 0, y, t2, coords, t1, t3, 0, $.JSNumber_methods);
          if (!(y > t2 - t3 / 2)) {
            t1 = $.indexOf$1$asx(t1.$index(t1, "history"), coords);
            if (typeof t1 !== "number")
              return this.isCollision$2$bailout(8, 0, 0, 0, coords, t1);
            if (!(t1 >= 0)) {
              t1 = this.player;
              t1 = $.indexOf$1$asx(t1.$index(t1, "history"), coords);
              if (typeof t1 !== "number")
                return this.isCollision$2$bailout(9, 0, 0, 0, 0, t1);
              t1 = t1 >= 0;
            } else
              t1 = true;
          } else
  ...
  isCollision$2$bailout: function(state0, x, y, t2, coords, t1, t3, t4, t5, t6) {
    switch (state0) {
      case 0:
      case 1:
        state0 = 0;
      ...
      default:
        if (state0 === 9 || state0 === 8 || state0 === 7 || state0 === 6 || state0 === 5 || state0 === 4 || state0 === 3 || state0 === 0 && !t3.$lt(x, $.$div$n(t2, 2)))
          switch (state0) {
            case 0:
              t2 = this.canvas;
            ...
            case 3:
              state0 = 0;
              t6 = t1.$index(t1, "width");
            case 4:
              state0 = 0;
            default:
              if (state0 === 9 || state0 === 8 || state0 === 7 || state0 === 6 || state0 === 5 || state0 === 0 && !t3.$gt(x, $.$sub$n(t5, $.$div$n(t6, 2))))
                switch (state0) {
                  case 0:
                    t3 = t1.$index(t1, "height");
                  case 5:
                  ...
                  default:
                    if (state0 === 9 || state0 === 8 || state0 === 7 || state0 === 6 || state0 === 0 && !t5.$lt(y, $.$div$n(t3, 2)))
                      switch (state0) {
                        case 0:
                          t2 = t4.get$height(t2);
                        case 6:
                        ...
                        case 7:
                          state0 = 0;
                        default:
                          if (state0 === 9 || state0 === 8 || state0 === 0 && !t5.$gt(y, $.$sub$n(t2, $.$div$n(t3, 2))))
                            switch (state0) {
                              case 0:
                                t1 = $.indexOf$1$asx(t1.$index(t1, "history"), coords);
                              case 8:
                                state0 = 0;
                              case 9:
                                if (state0 === 9 || state0 === 0 && !$.$ge$n(t1, 0))
                                  switch (state0) {
                                    case 0:
                                      t1 = this.player;
                                      t1 = $.indexOf$1$asx(t1.$index(t1, "history"), coords);
  ...
  inverseDirection$0: function() {
    var t1 = this.player;
    switch (t1.$index(t1, "current_direction")) {
      case "up":
        return "down";
      case "down":
        return "up";
...
$$.game_falsestart_anon = {"": "Closure;this_0",
  call$1: function(e) {
    ...
    if ($.$ge$n($.JSArray_methods.indexOf$1(["up", "down", "left", "right"], lastKey), 0) && !$.$eq(lastKey, this.this_0.inverseDirection$0())) {
      t1 = this.this_0.get$player();
      t1.$indexSet(t1, "current_direction", lastKey);
    } else if ($.$ge$n($.JSArray_methods.indexOf$1(["start_game"], lastKey), 0) && this.this_0.get$over()) {
      t1 = this.this_0;
      t1.resetPlayer$0();
      t1.resetEnemy$0();
      t1.set$over(false);
...
$$.game_falsestart_anon0 = {"": "Closure;this_1",
  call$1: function(t) {
    var t1 = this.this_1;
    if (!t1.get$over()) {
      t1.move$2(t1.get$player(), t1.get$enemy());
      t1.draw$1(t1.get$player());
      t1.moveEnemy$0();
      t1.draw$1(t1.get$enemy());
    }
    return;
  }
};
...
$.game$ = function() {
  return new $.game($.query("#the-game"), null, $.makeLiteralMap(["type", "program", "width", 16, "height", 16, "patern", "body2", "history", [], "y", 0, "x", 0, "current_direction", null]), $.makeLiteralMap(["type", "user", "width", 16, "height", 16, "patern", "body", "history", [], "y", 0, "x", 0, "current_direction", null]), $.makeLiteralMap(["up", [38, 87], "down", [40, 83], "left", [37, 65], "right", [39, 68], "start_game", [13, 32]]), null, false);
};
